...
export const Navigation = ({burgerMenuOpen}: INavigation) => {
    ...
    const activeLinkColor = '#e08821'
    const activePage = ( {isActive}: any ) => { return isActive ? {color : activeLinkColor} : undefined };
            ...
            {links.map((link: ILink , index: number)=>{
                return (
                    <div key={index} className='link-container' onClick={()=>navigationHandler(link.path)}>
                        <NavLink to={link.path} style={activePage}>{link.text}</NavLink>
